...
월별 무역수지에서 단기간에 발생한 급격한 변화를 확인하기 위해 전월 대비 무역수지 변화량을 계산하였다.

무역수지는 흑자와 적자를 오가며 음수 값도 가질 수 있기 때문에 일반적인 백분율 변화율을 적용할 경우 해석이 왜곡될 수 있다. 따라서 본 분석에서는 전월과 비교한 무역수지 금액의 차이를 사용하였다.

이 분석은 장기 추세를 확인하는 12개월 이동평균과 달리 특정 월에서 발생한 단기적인 충격과 변곡점을 확인하기 위해 적용하였다.

### 5.4 계절성 확인

장기적인 트렌드와 반복적인 계절성을 구분하기 위해 2016~2025년의 같은 월을 묶어 월별 평균 무역수지를 계산하였다.

12개월 이동평균은 여러 달에 걸쳐 지속되는 장기적인 상승·하락 흐름을 확인하는 데 사용하였고, 월별 평균 비교는 매년 같은 시기에 비슷한 변화가 반복되는지를 확인하는 데 사용하였다.

특정 월의 평균 무역수지가 다른 월보다 반복적으로 높거나 낮게 나타나는 경우 계절성의 가능성을 검토하였다.
...
월별 분석 결과가 특정 한 달의 영향에만 의해 나타난 것인지 확인하기 위해 데이터를 분기 단위로 다시 집계하였다.

분기별 무역수지 결과를 월별 분석 결과와 비교하여 2022~2023년에 나타난 무역수지 악화가 단일 월의 이상값이 아니라 여러 분기에 걸쳐 지속된 흐름인지 확인하였다.

이를 통해 월별 분석에서 확인한 주요 추세가 다른 집계 단위에서도 유지되는지 재검증하였다.
...
분석 결과처럼 무역수지가 단기간에 크게 악화되는 상황을 확인하기 위해서는 수출액뿐 아니라 수입액의 변화도 함께 모니터링할 필요가 있다.

특히 수입 증가가 무역수지 악화와 함께 나타나는 시기에는 국제 원자재 및 에너지 가격, 환율 등의 변화를 추가적으로 확인하여 수입 비용 증가 위험을 점검할 수 있다.

기업의 경우 이러한 지표를 원자재 조달과 수입 비용 관리 시 참고자료로 활용할 수 있다.
